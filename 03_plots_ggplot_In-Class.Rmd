...
ggplot(custdata, aes(x = log10(age))) +
  geom_density(fill = "purple", alpha = 0.5) +
  labs(title = "Log10 Transformed Density Plot of Age", 
       x = "log10(Age)", 
       y = "Density")
...
```{r bar_chart}
ggplot(custdata, aes(x = marital_status, fill = as.factor(health_ins))) +
  geom_bar(position = "dodge") +
  labs(title = "Side-by-Side Bar Chart: Health Insurance by Marital Status", 
       x = "Marital Status", 
       y = "Count", 
...
ggplot(health_ins_count_by_marital_status, aes(x = marital_status, y = ct, fill = as.factor(health_ins))) +
  geom_bar(stat = "identity", position = "dodge") +  # Creates a bar chart
  geom_text(aes(label = ct), vjust = -0.5, position = position_dodge(0.9), size = 4) +  # Adds labels
  labs(title = "Health Insurance Count by Marital Status", 
       x = "Marital Status", 
       y = "Count", 
       fill = "Health Insurance") +
  theme_minimal()
